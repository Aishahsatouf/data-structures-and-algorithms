...
const templatingWithMustache = () => {
  // Solution code here...
   ...
   characters.forEach(item=>{
    let page2 = $('#template').html();
   html.push( Mustache.render(page2, item));
 })
  return html 
...
const hasChildrenValues = (arr, character) => {
  ...
  if (character === element.name){
    let valuesData = Object.values(element);
    if (valuesData[2].length>0){
      returned = true;
    }
  }
  })
...
const houseSize = (arr) => {
  ...
  arr.forEach(element=>{
    ...
    if(element.spouse){
      ...
      members+=element.children.length;
    }
