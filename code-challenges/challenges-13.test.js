...
const standardizePhoneNumbers = (arr) => {
  ...
  let array = [];
  arr.forEach(item => {
    array.push(item.replace(/[()-]/g, '').split(' ').join(''));
  })
  return array
};
...
const allHappy = (arr) => {
  // Solution code here...
  let number = arr.reduce((acc, val, idx) => {
    if (val.includes(':)')) {
      acc++;
    }
    return acc
...
const findEvery = (arr, target) => {
  // Solution code here...
  let numb = arr.reduce((acc, val, idx) => {
    if (val.includes(target)) {
      acc++;
    }
    return acc
  }, 0)
  return numb === arr.length;
};
...
const sortByDay = (arr) => {
  ...
  let Saturday=[];
  let Sunday=[];
  let allarrays=[Monday,Tuesday,Wednesday,Thursday,Friday,Saturday,Sunday];
  arr.forEach(item=>{
    daysOfWeek.forEach((elem,idx)=>{
      if(item.includes(elem)){
        allarrays[idx].push(item);
      }
    })
  })
  return allarrays;
