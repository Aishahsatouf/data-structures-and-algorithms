...
const filterStringsWithVowels = (arr) => {
  ...
  let VowelIncludes =arr.filter(n=>{
    if(n.match(/[aeiou]/ig)){
      return true;
    }
  })
  return VowelIncludes
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  let nOtIncludes=arr.filter(element=>{
...
const getStatName = (arr, minBaseStat) => {
  ...
  let nameArray =objArray.map(element=>{
...
const evenOddNumericValues = (arr) => {
  ...
  let evenOddArray=numArray.map(num=>{
    if(num%2===0){
      return 'even';
    }else if(num%2 !==0){
      return 'odd';
