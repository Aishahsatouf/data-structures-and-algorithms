...
const generateSubmitButton = () => {
  // Solution code here...
  $('section').prepend("<button>submit</button>");
}
...
const isCapitalized = (str) => {
  // Solution code here...
  let tested =/\b[A-Z]\w*/g;
 let  result = str.match(tested);
 if (result===null){
   return result=[];
 }
  return result;
...
const matchMonth = (input) => {
  // Solution code here...
  let tested = /\b[Oo]ct(ober)*\b/g;
  if (tested.test(input)){
    return true;
  }else{
    return false;
  }
...
let hangman = (str) => {
  // Solution code here...
  let toReplace= /[aeiou]/gi;
  return(str.replace(toReplace,'_'));
};
