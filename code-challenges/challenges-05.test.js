...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach(item =>{
    ...
    let template =$('#template').first().clone();
    let h2= template.find('h2');
    h2.text(item.name);
    let h3=template.find('h3');
    h3.text(item.height);
    let p =template.find('p');
    p.text(item.eye_color);
...
const listFoods = (recipe) => {
  ...
  let indexOfSpace = item.indexOf(' ',3);
  let included= item.slice(indexOfSpace+1,item.length);
...
const splitFoods = (recipe) => {
  ...
  // Solution code here...
  recipe.ingredients.forEach(item=>{
    let newArray = item.split(' ').slice(2);
...
const stepActions = (recipe) => {
 ...
 for(let i=0;i<recipe.steps.length;i++){
   result.push(recipe.steps[i].split(' ').shift());
 } 
  return result;
};
