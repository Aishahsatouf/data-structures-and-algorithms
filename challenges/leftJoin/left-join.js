function leftJoin(hashmap1, hashmap2) {
  ...
  for (let i = 0; i <= hashmap1.hash.length - 1; i++) {

      if (hashmap1.hash[i]) {
          let key = [Object.keys(hashmap1.hash[i].head.value)[0]];
          let value = [Object.values(hashmap1.hash[i].head.value)[0]]
          results.push([key[0], value[0]]);

          if (hashmap1.hash[i].head.next !== null) {
              let current = hashmap1.hash[i].head.next;
